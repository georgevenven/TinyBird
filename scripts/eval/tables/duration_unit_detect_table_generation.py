...
def main():
    ap = argparse.ArgumentParser(
        description="Generate min/max duration summary table for unit-detection sweep."
    )
    ap.add_argument(
        "eval_csv",
        nargs="?",
        default="results/duration_classify_unit_detection_sweep/eval_f1.csv",
        help=(
            "Path to unit-detect sweep eval_f1.csv "
            "(default: results/duration_classify_unit_detection_sweep/eval_f1.csv)"
        ),
    )
    ap.add_argument(
        "--out_csv",
        default=None,
        help="Output CSV path (default: <eval_csv_dir>/duration_unit_detect_min_max_table.csv)",
    )
    ...
    out_csv = (
        ...
        / "duration_unit_detect_min_max_table.csv"
    )
