...
def _spec_keys_from_stem(stem_lc):
    """Exact keys from spec filename stem like '131_bird8' -> (clip=131, bird=8)."""
    b = re.search(r'bird[_-]?(\d+)', stem_lc, re.I)
    bird = int(b.group(1)) if b else None
    c = re.search(r'^(\d+)', stem_lc) or re.search(r'(\d+)', stem_lc)
    clip = int(c.group(1)) if c else None
    return clip, bird

def load_bengal_events(json_path, selected_bird=None):
    """
    Parse Bengal finch JSON and return a list of entries for fuzzy matching.
    GEORGE SAYS THIS IS TEMPORARY: we will do sloppy substring matching on filename stem and bird_id.
    {
      "metadata": {"units":"ms"},
      "recordings":[
    ...
    GEORGE SAYS THIS IS TEMPORARY: replace with a proper manifest or exact mapping later.
    
    Args:
        json_path: Path to the JSON file
        selected_bird: If provided, only include events from this bird number (e.g., 1 for bird1)
    """
    ...
    for rec in jd.get("recordings", []):
        fn = rec.get("recording", {}).get("filename", "") or ""
        bird = rec.get("recording", {}).get("bird_id", "") or ""
        stem = Path(fn).stem
        clip_num = _first_int(stem)          # '0.wav' -> 0
        bird_num = _first_int(bird)          # 'Bird1' -> 1
        if clip_num is None or bird_num is None:
            continue  # skip unusable entries
        # Filter by selected bird if specified
        if selected_bird is not None and bird_num != selected_bird:
            continue
        ev_list = []
        ...
        event_map.setdefault((bird_num, clip_num), []).extend(ev_list)
    return event_map
...
def main(args):
    ...
    if args.get("json_path"):
        event_map = load_bengal_events(args["json_path"], selected_bird=args.get("bird"))
    ...
    while i < len(embedding_dataset) and total_timebins < args["num_timebins"]:
        ...
        # Exact matching: derive (clip,bird) from spec filename stem
        stem_lc = Path(file_name).stem.lower()
        spec_clip, spec_bird = _spec_keys_from_stem(stem_lc)
        key = (spec_bird, spec_clip)
        matched_events = event_map.get(key, [])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bird", type=int, default=None, help="select specific bird number (e.g., 1 for bird1, 2 for bird2)")
