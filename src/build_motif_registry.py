...
class MotifRegistry:
    ...
    def ensure_metadata(self, meta: Optional[dict]) -> bool:
        ...
        if normalized is None:
            logging.warning(
                "PT metadata missing required fields (%s); skipping file.", ", ".join(self.REQUIRED_META_FIELDS)
            )
            return False
        ...
        for key, expected_value in current.items():
            ...
            if isinstance(expected_value, float):
                if not math.isclose(float(candidate), float(expected_value), rel_tol=1e-9, abs_tol=1e-12):
                    logging.warning(
                        "Metadata mismatch (%s): expected %s, found %s. Skipping.", key, expected_value, candidate
                    )
                    return False
            else:
...
def find_motifs(T, m, n_new_motifs=3, max_seed_distance=3.5, dataset_id="T", motifs=None):
    ...
    def _match_pattern(pattern, length, dim_index):
        ...
        return {"seed_index": seed_index, "seed_distance": seed_distance, "starts": starts}
...
def process_pt_file(
    ...
    for channel_index in range(channels):
        ...
        if intervals.size == 0:
            logging.info("Skipping channel %d for %s (no chirp intervals).", channel_index, file_path.name)
            continue
        ...
        if chirp_tensor.size == 0:
            logging.info("Skipping channel %d for %s (empty chirp tensor).", channel_index, file_path.name)
            continue
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--motif_length", type=int, default=40, help="Motif length (time bins).")
    parser.add_argument("--n_new_motifs", type=int, default=10, help="Maximum motifs to add per channel.")
    parser.add_argument(
        "--max_seed_distance",
        type=float,
        default=2.0,
